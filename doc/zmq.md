...
The Coppercoin Core daemon can be configured to act as a trusted "border
router", implementing the coppercoin wire protocol and relay, making
...
The ZeroMQ feature in Coppercoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building coppercoind:
...
For instance:
    ...
    $ coppercoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/coppercoind.tx.raw \
...
These options can also be provided in coppercoin.conf.
...
From the perspective of coppercoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into coppercoind directly. Furthermore, no information is
...
using. Coppercoind appends an up-counting sequence number to each
